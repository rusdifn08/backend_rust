...
pub async fn register(
    ...
    info!(
        "API CALL: [POST] /api/auth/register - Attempting to register user: {}",
        payload.email
    );
    ...
    .map_err(|e| {
        if e.to_string().contains("duplicate key") {
            error!(
                "Registration failed: Email or username already exists for {}",
                payload.email
            );
            (
                StatusCode::CONFLICT,
...
pub async fn login(
    ...
    info!(
        "API CALL: [POST] /api/auth/login - Login attempt for: {}",
        payload.username
    );
...
pub async fn update_profile(
    ...
    if let Ok(Some(old_user)) = sqlx::query_as::<_, User>("SELECT * FROM users WHERE id = $1")
        .bind(id)
        .fetch_optional(&pool)
        .await
    {
                ...
                if &old_url != new_url && old_url.contains("/api/files/") {
                    if let Some(file_id_str) = old_url.split("/api/files/").last() {
                        if let Ok(file_uuid) = uuid::Uuid::parse_str(file_id_str) {
                            let _ = sqlx::query("DELETE FROM file_uploads WHERE id = $1")
                                .bind(file_uuid)
                                .execute(&pool)
                                .await;
                        }
