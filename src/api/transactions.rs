...
pub async fn get_transactions(
    ...
    let parsed_uuid = Uuid::parse_str(&user_id).map_err(|_| (StatusCode::BAD_REQUEST, "Invalid UUID format".to_string()))?;
    let txs = sqlx::query_as::<_, Transaction>("SELECT * FROM transactions WHERE user_id = $1 ORDER BY created_at DESC")
        .bind(parsed_uuid)
        .fetch_all(&pool)
        .await
