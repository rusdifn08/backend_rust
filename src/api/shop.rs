...
pub async fn get_shop_items(State(state): State<AppState>) -> impl IntoResponse {
...
pub async fn buy_item(
    ...
    let item = match sqlx::query_as::<_, ShopItemPrice>(
        r#"SELECT price, max_purchases FROM shop_items WHERE id = $1"#,
    )
    ...
    // Check user coins
    #[derive(sqlx::FromRow)]
    struct UserStatsCoins {
        coins: i32,
    }
    let stats = match sqlx::query_as::<_, UserStatsCoins>(
        r#"SELECT coins FROM user_stats WHERE user_id = $1"#,
    )
    ...
    .await
    {
        ...
        Ok(None) => {
            return (
                StatusCode::NOT_FOUND,
                Json(json!({ "success": false, "error": "User stats not found" })),
            )
                .into_response()
        }
        Err(e) => {
    ...
    if stats.coins < total_cost {
        return (
            ...
            Json(json!({ "success": false, "error": "Not enough coins" })),
        )
            .into_response();
    }
    ...
    let current_inv = sqlx::query_as::<_, InvQty>(
        r#"SELECT quantity FROM user_inventory WHERE user_id = $1 AND item_id = $2"#,
    )
    .bind(payload.user_id)
    .bind(payload.item_id)
    .fetch_optional(&mut *tx)
    ...
    if let Some(max_p) = item.max_purchases {
        ...
        if current_qty + payload.quantity > max_p {
            return (
                StatusCode::BAD_REQUEST,
                Json(json!({ "success": false, "error": "Purchase limit reached" })),
            )
                .into_response();
        }
    }
    ...
    if current_inv.is_some() {
        ...
        let _ = sqlx::query(
            r#"INSERT INTO user_inventory (user_id, item_id, quantity) VALUES ($1, $2, $3)"#,
        )
        .bind(payload.user_id)
        .bind(payload.item_id)
        .bind(payload.quantity)
    ...
    if tx.commit().await.is_err() {
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({ "success": false, "error": "Transaction failed" })),
        )
            .into_response();
    }
    ...
    (
        StatusCode::OK,
        Json(json!({ "success": true, "message": "Purchase successful" })),
    )
        .into_response()
}
...
pub async fn equip_item(
    ...
    .await
    {
        ...
        Ok(None) => {
            return (
                StatusCode::NOT_FOUND,
                Json(json!({ "success": false, "error": "Inventory item not found" })),
            )
                .into_response()
        }
        Err(e) => {
    ...
    (
        StatusCode::OK,
        Json(json!({ "success": true, "message": "Equipped state updated" })),
    )
        .into_response()
}
